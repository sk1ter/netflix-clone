...
class APICaller {
    ...
    func getPopularMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.API_URL)/3/movie/popular?api_key=\(Constants.API_KEY)") else {return}
    ...
    func getUpcomingMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.API_URL)/3/movie/upcoming?api_key=\(Constants.API_KEY)") else {return}
    ...
    func getTopRatedMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.API_URL)/3/movie/top_rated?api_key=\(Constants.API_KEY)") else {return}
    ...
    func search(query: String, completion: @escaping (Result<[Title], Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constants.API_URL)/3/search/multi?api_key=\(Constants.API_KEY)&language=en-US&page=1&include_adult=false&query=\(query)") else {return}
    ...
    func getMovie(with query: String,  completion: @escaping (Result<VideoElement, Error>) -> Void) {
        guard let query = query.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else {return}
        guard let url = URL(string: "\(Constants.YOUTUBE_API_URL)/search?q=\(query)&key=\(Constants.GOOGLE_API_KEY)") else {return}
